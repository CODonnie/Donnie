...
const Welcome = () => {
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      ...
      {/* About element */}
      <div className='fixed bottom-50 left-[7%] flex flex-col items-center justify-end gap-0 h-[120px] cursor-pointer'>
      ...
      <div className="intro fixed top-55 left-20">
        <div className='group relative w-90'>
          <div className="flex gap-4 absolute top-3 left-1.5">
            <p>Hi, I'm</p>
            <p className='relative'>Ordu Donald Chibuike</p>
          </div>
          <h2 className="text-[120px] font-black tracking-tighter">DONNIE</h2>
          <p className="italic text-muted text-[15px] font-semibold absolute top-[80%] left-2">FrontEnd Developer, UI/UX & Graphics Designer</p>
        </div>
      </div>

      {/* project list n filter button */}
      <div className='mt-35 mr-5 sm:mr-20 h-full overflow-y-auto scrollbar-hide'>
        {/* filter buttons */}
        <div className='w-65 flex gap-2 fixed top-24 right-15 justify-end'>
          {
        ...
        <div className='mb-100'>
          {getProject.map((items, index) => (
            <div key={index} className="flex flex-col items-end mx-4">
              <div className='flex items-center justify-end group h-[80px]'>
                ...
                <h2
                  ...
                  className="sm:text-[5rem] text-[4rem] cursor-pointer font-extrabold leading-tight italic animate-bounceTwice transition-all"
                >
                  {items.title}
                </h2>
              </div>
